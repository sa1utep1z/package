...
const CreateOrderApi = {
  ...
  GetType: async (params) => await httpRequest.post('admin/app/order/total', params), //获取订单各状态数字；
  ...
  CreateBasicOrder: async (params) => await httpRequest.post('admin/app/order/basic', params), //订单基本信息；
  getBasicOrder: async (orderId) => await httpRequest.get(`admin/app/order/${orderId}/basic`), //查询订单基本信息；
  getOriginPhotos: async () => await httpRequest.get(`admin/app/order/images`), //查询订单基本信息初始职位图片；

  RequirementOrder: async (params) => await httpRequest.post('admin/app/order/requirement', params), //录用要求；
  getRequirementOrder: async (orderId) => await httpRequest.get(`admin/app/order/${orderId}/requirement`), //查询录用要求信息；

  PolicyRequirement: async (params) => await httpRequest.post('admin/app/order/applyDetail', params), //接单政策说明；
  getPolicyOrder: async (orderId) => await httpRequest.get(`admin/app/order/${orderId}/applyDetail`), //查询接单政策说明信息；

  CommissionDescription: async (params, orderId) => await httpRequest.post(`admin/app/empReward/${orderId}`, params), //招聘员提成说明；
