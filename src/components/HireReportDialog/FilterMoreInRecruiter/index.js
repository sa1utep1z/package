...
const FilterMoreInRecruiter = () => {
  ...
  const renderReCruiterItem = ({item}) => {
    const isSelected = selectRecruiter.value === item.value;
    return (
      <TouchableOpacity style={{height: 30, borderWidth: 1, borderTopWidth: 0, borderColor: '#EFEFEF', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingLeft: 10}} onPress={() => recruiterOnPress(item)}>
        <Text style={{color: '#333333'}}>{item.label}</Text>
        <CheckBox
          checked={isSelected}
          size={18}
      ...
      <View style={{maxHeight: 450, paddingHorizontal: 10}}>
        ...
        <>
          <TouchableOpacity style={[styles.touchArea, showRecruiter && styles.selectedTouchArea, {marginTop: 10}]} onPress={changeWay}>
            <Text numberOfLines={1} style={[{fontSize: 16, color: '#000'}, showRecruiter && styles.fontBold]}>{`${selectRecruiter.value ? `已选招聘员：${selectRecruiter.title}` : '请选择招聘员'}`}</Text>
          </TouchableOpacity>
