      ...
      if (range) {
        if (selecting) {
          if (_date.isBefore(startDate, "day")) {
            onChange({ startDate: _date, endDate: null });
          } else {
    ...
    <Modal transparent visible={isOpen}>
      <View style={mergedStyles.backdrop}>
        ...
        <View style={mergedStyles.container}>
          ...
          {range && (
            <View style={{height: 50, borderTopWidth: 1, flexDirection: 'row', borderColor: '#E3E3E3'}}>
              <TouchableOpacity style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={() => {
                display && display(false);
              }}>
                <Text style={{fontSize: 18, color: '#999999'}}>取消</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{flex: 1, backgroundColor: '#409EFF', justifyContent: 'center', alignItems: 'center', borderBottomRightRadius: 6}} onPress={() => {
                confirmBtn();
                display && display(false);
              }}>
