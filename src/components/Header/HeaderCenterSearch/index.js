...
const HeaderCenterSearch = ({routeParams}) => {
  ...
  const check = (name) => {
    if(['INTERVIEW_LIST', 'LEAVING_LIST', 'SIGN_UP_LIST', 'WAIT_TO_ENTRY_LIST', 'NEWEST_STATE', 'MY_MEMBERS', 'DATA_STATISTICS', 'LEAVING_MANAGE', 'ORDER_MANAGE', 'COMPLAINT_FEEDBACK', 'ADVANCE_MANAGE', 'DORMITORY_LIST', 'DORMITORY_CHECK_LIST', 'DORMITORY_VIOLATION', 'DORMITORY_DATA'].includes(name.toUpperCase())){
      return 'list';
