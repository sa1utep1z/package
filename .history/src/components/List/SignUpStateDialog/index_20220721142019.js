...
const SignUpStateDialog = ({
  ...
  const selectedStatusOnPress = (stage) => {
    setStage(stage);
    setShowStage(false);
  }
    ...
    <Dialog
      ...
      <View style={styles.msgArea}>
        ...
        <View style={{width: '100%'}}>
          ...
          {showStage && CHANGING_STAGE_LIST_IN_DIALOG.map((stage, stageIndex) => {
            return (
              <TouchableOpacity key={stageIndex} style={[{
                ...
                padding: 10, 
                height: 40
              }, stageIndex === CHANGING_STAGE_LIST_IN_DIALOG.length - 1 && {borderBottomLeftRadius: 8, borderBottomRightRadius: 8, marginBottom: 10 }]} onPress={()=>selectedStatusOnPress(stage)}>
          ...
          {newestStatus && stage?.statusList.length && stage.statusList.map((status, statusIndex) => {
            return (
              <TouchableOpacity key={statusIndex} style={[{
                ...
                padding: 10, 
                height: 40
              }, statusIndex === CHANGING_STAGE_LIST_IN_DIALOG.length - 1 && {borderBottomLeftRadius: 8, borderBottomRightRadius: 8, marginBottom: 10 }]} onPress={()=>selectedStatusOnPress(status)}>
