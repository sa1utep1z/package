...
import { DEFAULT_STATUS_LIST, CHANGING_STATUS_LIST_IN_DIALOG } from '../../../utils/const';
...
const SignUpStateDialog = ({
  ...
  const newestStatusOnPress = () => setNewestStatus(!newestStatus);

  const selectedStatusOnPress = (status) => {
    setStage(status);
    setShowStage(false);
  }
    ...
    <Dialog
      ...
      <View style={styles.msgArea}>
        ...
        <View style={{width: '100%'}}>
          ...
          {showStage && CHANGING_STATUS_LIST_IN_DIALOG.map((status, statusIndex) => {
            return (
              <TouchableOpacity key={statusIndex} style={[{
                ...
                padding: 10, 
                height: 40
              }, statusIndex === CHANGING_STATUS_LIST_IN_DIALOG.length - 1 && {borderBottomLeftRadius: 8, borderBottomRightRadius: 8, marginBottom: 10 }]} onPress={()=>selectedStatusOnPress(status)}>
          ...
          {stage && (
            <TouchableOpacity style={[{
              ...
              height: 40
                ...
                <Text style={[styles.tagArea_title]}>{'请选择状态'}</Text>
            </TouchableOpacity>
          )}
          {newestStatus && <View style={{width: '100%', borderWidth: 1, borderTopWidth: 0, borderColor: '#EFEFEF', borderBottomLeftRadius: 8, borderBottomRightRadius: 8, marginBottom: 10, padding: 10, paddingBottom: 0, paddingRight: 0}}>
            <View style={[styles.tags_little, {borderWidth: 1}]}>
              {statusList.length && statusList.map((item, index) => {
                const isFill = statusList.length % 3 === 2;
                const isLastElement = index === statusList.length - 1;
                return (
