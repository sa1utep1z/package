...
const SignUpStateDialog = ({
  ...
  const statusOnPress = () => setChangingStatus(!changingStatus);

  const selectedStatusOnPress = (status) => setSelectedStatus(status);
  ...
  return (
    <Dialog
      ...
      <View style={styles.msgArea}>
        ...
        <View style={{width: '100%', minHeight: 100, paddingBottom: 10}}>
          <TouchableOpacity style={[{
            ...
            height: 40
              ...
              <Text style={[styles.tagArea_title]}>{selectedStatus ? selectedStatus.title : '选择阶段'}</Text>
                
          </TouchableOpacity>
          {changingStatus && CHANGING_STATUS_LIST_IN_DIALOG.map((status, statusIndex) => {
            return (
              <TouchableOpacity key={statusIndex} style={[{
                ...
                padding: 10, 
                height: 40
              }, statusIndex === CHANGING_STATUS_LIST_IN_DIALOG.length - 1 && {borderBottomLeftRadius: 8, borderBottomRightRadius: 8 }]} onPress={()=>selectedStatusOnPress(status)}>
