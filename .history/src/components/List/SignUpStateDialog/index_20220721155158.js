    ...
    <Dialog
      ...
      <View style={styles.msgArea}>
        ...
        <View style={{width: '100%'}}>
          ...
          {showNewestStatus && stage?.statusList?.length && stage.statusList.map((statusInList, statusIndex) => {
            return (
              <TouchableOpacity key={statusIndex} style={[{
                ...
                <Text>{statusInList.title}</Text>
                {statusInList.value === newestStatus?.value && <CheckRadio checked/>}
              </TouchableOpacity>
            )
          })}
