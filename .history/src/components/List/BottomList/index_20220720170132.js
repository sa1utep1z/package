      ...
      <Tab
        ...
        {tabList.map((tab, tabIndex) => {
          ...
          return (
            ...
            //   <Text style={[active && {color: '#409EFF', fontWeight: 'bold'}]} onPress={setIndex}>{tab.title}<Text style={[{fontWeight: 'bold'}, active && {color: '#409EFF'}]}> ({list.length})</Text></Text>
            // </TouchableOpacity>
              <Tab.Item
                title={tab.title}
                key={tab.type}
                titleStyle={[styles.tabItem_titleStyle, active && styles.tabItem_titleStyle_active, {borderWidth: 1, width: '100%', paddingVertical: 0}]}
                buttonStyle={[styles.tabItem_buttonStyle, {borderWidth: 1, paddingHorizontal: 0}]}
                containerStyle={{borderWidth: 1, height: 50, justifyContent: 'center', paddingHorizontal: 0}}
              />
          )
        })}
      </Tab>
