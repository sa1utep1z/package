...
const CompanyListDialog = (props, ref) => {
  ...
  const { isLoading, data, isError, error, refetch } = useQuery(['companyList', params], HomeApi.HomePage);
    ...
    <Dialog
      ...
      <View style={styles.listArea}>
        ...
        <View style={styles.listHead}>
          ...
          <TouchableOpacity style={[styles.gotoDetail, styles.flex_1]}>
        ...
        <ScrollView style={styles.scrollArea}>
          {DATA?.length && DATA.map((item, index) => {
            ...
            return (
              <View style={[styles.listItem, isLastIndex && styles.noBorder]} key={item.index}>
                <Text style={[styles.item, styles.flex_1]}>{item.index}</Text>
                <Text style={[styles.item, styles.flex_4]}>{item.title}</Text>
