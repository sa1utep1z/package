      ...
      {data?.data?.length && data.data.map((item, index)=>{
        ...
        return (
          <View style={styles.itemArea}>
            <View style={styles.text}>姓名：</View>
            <View style={styles.text}>微信号：{item.weChatName || '无'}</View>
            <View style={styles.text}>手机号：{item.mobile}</View>
            <View style={styles.text}>身份证号：{item.idNo}</View>
            <View style={styles.text}>位置信息：</View>
            <View style={styles.text}>会员来源：</View>
            <View style={[styles.text, {borderBottomWidth: 0}]}>注册日期：{day}</View>
