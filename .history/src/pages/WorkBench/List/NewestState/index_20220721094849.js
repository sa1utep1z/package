...
import { MEMBER_INFO, TAB_OF_LIST, RENDER_ITEM_OF_LIST } from "../../../../utils/const";
...
const NewestState = () => {
  ...
  const renderItem = ({item}) => {
    return (
      <View key={item.id} style={styles.listStyle}>
        <TouchableOpacity key='name' style={{flex: 1, justifyContent: 'center', alignItems: 'center', flexDirection: 'row'}} onPress={callMemberPhone}>
          <Text style={[{fontSize: 12}, styles.canPress]}>{item.name}</Text>
          <Entypo
            name='phone'
            size={16}
            color='#409EFF'
        ...
        <TouchableOpacity key='company' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={showFactoryDetail}>
          <Text style={{fontSize: 12}}>{item.factory}</Text>
        </TouchableOpacity>
        <TouchableOpacity key='twoCard' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={showMemberDetail}>
          <Text style={{fontSize: 12}}>{item.card}</Text>
        </TouchableOpacity>
        <TouchableOpacity key='status' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={showSignUpDetail}>
          <Text style={{fontSize: 12}}>{item.state}</Text>
        </TouchableOpacity>
        <TouchableOpacity key='record' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={gotoRecordOfWorking}>
          <Text style={[{fontSize: 12}, styles.canPress]}>查看</Text>
        </TouchableOpacity>
        <TouchableOpacity key='from' style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} onPress={showMemberDetail}>
          <Text style={[{fontSize: 12}, styles.canPress]}>查看</Text>
        </TouchableOpacity>
      </View>
    )
  ...
  return (
    <View style={[styles.screen, showSearch && {paddingTop: 10}]}>
      ...
      <BottomList 
        list={list}
        renderItem={RENDER_ITEM_OF_LIST.NEWEST_STATE}
