...
const Login = (props) => {
  ...
  const rememberToastRef = useRef(null);
  ...
  const rememberOnPress = () => {
    setRemember(!remember);
    const {current: {info}} = rememberToastRef;
    if(!remember){
      info("保存密码有效期为一个月", 0.5);
    }
  };
            ...
            {({handleSubmit}) => (
              <View style={styles.formArea}>
                ...
                <CheckBox
                  ...
                  containerStyle={{backgroundColor: 'rgba(0,0,0,0)', padding: 0}}
                />
                <Toast ref={rememberToastRef}/>
                <Button
                  title="登 录"
