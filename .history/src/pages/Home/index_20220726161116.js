...
const Home = (props) => {
  ...
  const results = useQueries({
    queries: [
      {queryKey: ['homePage', searchContent], queryFn: HomeApi.HomePage},
      {queryKey: ['companyList', selectItem], queryFn: HomeApi.CompanyList}
    ]
  });
  ...
  const refreshControl = (
    <RefreshControl
      refreshing={results[0]?.isLoading}
      onRefresh={results[0]?.refetch}
    />
  );
  // data?.data?.content
  return(
    <>
      <FlatList
        data={results[0].data.data.content}
